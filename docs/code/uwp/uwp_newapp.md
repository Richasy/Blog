...
现在界面中央会出现一个对半分的界面，上面是预览，下面是代码，这里就是我们写界面的地方了。通常情况下，我们是不会让这两个窗口同时存在的，这太占地方了！
...
这里就有一个玄机了。你会发现，App.xaml的前面有一个三角符号，点一下它，App.xaml的下面多出来一个文件，名为App.xaml.cs，双击打开它。
